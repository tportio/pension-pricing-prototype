...
### 1단계: 코드 수정하기
코드 에디터(VS Code 등)에서 원하는 파일을 수정합니다.

### 2단계: 로컬에서 테스트하기
터미널에서 다음 명령어를 실행하여 수정 사항을 확인합니다:
```bash
npm run dev
```
브라우저에서 `http://localhost:5173`에 접속하여 정상 작동하는지 확인합니다.
확인이 끝나면 터미널에서 `Ctrl + C`를 눌러 종료합니다.

### 3단계: 변경사항 저장하기 (Git Commit)
터미널에서 다음 명령어를 순서대로 실행합니다:
```bash
git add .
git commit -m "수정 내용을 간단히 설명"
```

예시:
```bash
git commit -m "가격 정보 업데이트"
```

### 4단계: GitHub에 업로드하기 (Push)
```bash
git push
```
...
이 명령어를 실행하면 자동으로 빌드되고 GitHub Pages에 배포됩니다.
약 1-2분 후에 https://tportio.github.io/pension-pricing-prototype/ 에서 변경사항을 확인할 수 있습니다.
...
### ⚠️ 주의사항
- `npm run dev`로 테스트 후 문제가 없을 때만 배포하세요
- `git commit -m` 뒤의 메시지는 무엇을 수정했는지 간단히 작성하세요
- 배포 후 1-2분 정도 기다린 후 웹사이트를 확인하세요
